...
class TestController extends Controller
{
    ...
    public function studentContract($id_alumno_contrato, Request $request)
    {
        ...
        $sedeParams = [
            ...
            // array( 'id_depto' => '5', 'depto' => 'CAMPUS JULIACA', 'address'=>'Carretera Salida a Arequipa Km. 6 Chullunquiani, Autopista Heroes de la Guerra del Pacifico, provincia San Roman y departamento de Puno', 'place'=> 'Chullunquiani,', 'resp'=>'Dr.Raul Christian Acuña Salinas', 'respDocument'=>'10173524', 'court'=>'Corte Superior de Justicia de San Martin', 'apo'=> 'APODERADO'),
            // array( 'id_depto' => '6', 'depto' => 'CAMPUS TARAPOTO', 'address'=>'Urb. Santa Lucía, Jr. Los Mártires 340, Tarapoto, provincia San Martin y departamento de San Martin', 'place'=> 'Villa Unión,', 'resp'=>'Dr. Roussel Dulio Davila Villavicencio', 'respDocument'=>'01154650', 'court'=>'Corte Superior de Justicia de Puno', 'apo'=>'APODERADO'),
            // array( 'id_depto' => '8', 'depto' => 'ISTAT', 'address'=>'Villa Chullunquiani-Illapuso, altura Km. 6 de la carretera Arequipa, distrito de Juliaca, provincia de San Román, departamento de Puno', 'place'=> 'Chullunquiani,', 'resp'=>'Dr. Raul Christian Acuña Salinas', 'respDocument'=>'10173524', 'court'=>'Juliaca', 'apo'=> 'APODERADO'),
        ];
        $sedeParam = null;
        ...
        if ($valida == 'SI') {
            ...
            if (isset($adjunto_validate)) {
                ...
                try {
                    ...
                    $endPoint = '/api/enrollment/enrollment/enrollmentdetail/enrollment-steps';
                    $enrollmentData = null;
                    if (empty($request->no_img)) {
                        $enrollmentData = self::enrollmentHttpClient($endPoint, $params, $response);
                    }
                    //$photoStudent = self::getStudentPhoto($enrollmentData);
                    $meses = array("0", "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
                    $ordinal = array("ra", "da", "ra", "ta", "ta", "ta", "ma", "va", "na", "ma");
                    ...
                    $plain = collect(StudentData::planPago($contractAlumn->id_planpago_semestre))->first();
                    $info = collect(PaymentStudentInfoData::getAlumnContractDetail($id_alumno_contrato));
                    // $photoStudent = self::getStudentPhoto($info['foto'], 'minio-academic');
                    ...
                    $session = ['depto' => $id_depto, 'user' => $username];
                    ...
                    $doc = $getvalue['data'];
                    // validar que internamente -> 

                    //$data = ["datos_session" => $params_dato, "id_alumno_contrato" => $id_alumno_contrato, "content" => $content, "pathTemplate" => $pathTemplate, "id_user" => $id_user];
                    //$getvalue = AcademicAlumnoContratoData::validateTest($data);
    ...
    public function noOficial($id_alumno_contrato, Request $request)
    {
        //        $id_alumno_contrato = 297469;
        $jResponse = GlobalMethods::authorizationLamb($this->request);
        $code = $jResponse["code"];
    ...
    // public static function getStudentPhoto($url_foto)
    // {
    //     if (env('APP_ENV') != 'production') {

    //         if (!empty($url_foto)) {
    //             $sep = "lamb-academic/fotodb/";
    //             $picture = explode($sep, $url_foto);

    //             $slash = (count($picture) > 1 and $picture[1]) ? "lamb-academic/photos/fotodb/" . $picture[1] : "lamb-academic/photos/fotodb/" . 'sinfoto';
    //             $foto = $slash . ".jpg";
    //         } else {
    //             $foto = "lamb-academic/photos/fotodb/" . 'sinfoto' . ".jpg";
    //         }
    //     } else {
    //         $foto = $url_foto;
    //     }

    //     $storage = Storage::disk('minio-academic');
    ...
    //     }
    //     return $picture;
    //     //return 'https://files-erp.upeu.edu.pe/media/lamb-academic/fotodb/0380201121101.jpg';
    // }
    ...
    public function ScheduleListProgram($id_contrato_alumno) //Lista horas por cada tipo de horario
    {
        ...
        for ($i = 0; $i < $numHours; $i++) {
            if (
                $id_modulo . '-' . $id_tipo_formato_modulo . '-' . $codigo . '-' . $nombre . '-' . $fecha_inicio . '-' . $fecha_fin . '-' . $id_horario !=

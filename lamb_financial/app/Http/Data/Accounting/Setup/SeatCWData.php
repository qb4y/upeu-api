...
class SeatCWData extends Controller{
    private $request;
    ...
    public static function listCWSeatAaasinetIncome($id_nivel,$id_anho,$voucher){
        ...
        $query = "--QUERY 1
                ...
                and a.tipo in ('IC','ID')
                AND (a.importe <> 0 and upper(a.glosa) NOT LIKE upper('%<< Anulado>>%'))
                UNION ALL
                SELECT 
    ...
    public static function listCWSeatAaasinetSales($id_nivel,$id_anho,$voucher){
        ...
        $query = "SELECT a.serie,a.numvnt, 
                        ...
                        decode(a.tipo_mov,'01','CA','02','FA','09','PG','10','FA','12','FA','14','FA','16','FA','18','FA')||': (Doc:'||serie||'-'||to_number(a.numvnt)||')-'||substr(b.detalle,1,30)||' '||substr(ARON.apellido3(a.id_personal),1,20) descripcion,
                        a.id_mov_vnt memo,
                        a.docvnt
                FROM ARON.upeu_regventas a, ARON.upeu_regdetalle b 
                ...
                SELECT a.serie,a.numvnt, 
                        ...
                        '10' fondo, 							
                        nvl(decode(ARON.equiv(a.id_cuenta_cli,a.id_venta)||'-'||ARON.fc_cta_cte_aasinet(a.id_cuenta_cli,a.id_venta),'1132001-2','11010101',ARON.equiv_nivel(a.id_nivel_cont,a.id_venta)),' ') departamento,
                        nvl(ARON.fc_restriccion_aasinet(a.id_cuenta_cli,a.id_venta),' ')  restriccion, 							
                        to_char(a.importe*1,'99999999.99') valor, 
                        decode(a.tipo_mov,'01','CA','02','FA','09','PG','10','FA','12','FA','14','FA','16','FA','18','FA')||': (Doc:'||serie||'-'||to_number(a.numvnt)||')-'||substr(a.glosa,1,30)||' '||substr(ARON.apellido3(a.id_personal),1,20) descripcion,
                        a.id_mov_vnt memo,
                        a.docvnt
                FROM ARON.upeu_regventas a, ARON.upeu_regdetalle b 
                WHERE a.id_venta = b.id_venta 
    ...
    public static function listCWSeatAaasinetIMP($id_nivel,$id_anho,$voucher){
        ...
        $query = "SELECT 
                ...
                and a.voucher = '".$voucher."'
                and (a.base_imp <> 0 and upper(a.detalle) NOT LIKE upper('%<< Anulado>>%'))
                union all  
                SELECT   
                        '3151501' cuenta,   
                ...
                and to_char(a.fecha,'MM') = LPAD('".$mes_id."',2,0)
                and a.voucher = '".$voucher."'
                AND (A.importe <> 0 and upper(A.detalle) NOT LIKE upper('%<< Anulado>>%'))
                union all  
                SELECT  
                        nvl(ARON.fc_cta_aasinet(A.id_cuenta_cli,A.id_venta),' ') cuenta,  
                ...
                and A.voucher = '".$voucher."'
                and A.estado = 'V'
                AND (B.BASE_IMP <> 0 and upper(A.glosa) NOT LIKE upper('%<< Anulado>>%'))  	
    ...
    public static function listVoucherCWAasinetSalesOPT($id_user,$id_entidad,$id_depto,$id_anho,$id_mes){
        $id_compra = "001-".$id_anho;
        $query = "SELECT A.ID_VOUCHER AS VOUCHER,
                        to_char(A.fecha,'DD/MM/YYYY') FECHA,
                        A.ID_VOUCHER||' - '||to_char(A.FECHA,'DD/MM/YYYY') texto, 
                        to_char(A.fecha,'DD') dia, 
                        nvl(A.lote,'X') lote_ctr,
                        A.ID_NIVEL
    ...
    public static function listCWSeatAaasinetOPT($id_nivel,$id_anho,$voucher){
        ...
        $query = "SELECT a.serie,a.NUMDOC,4 ord, 
                ...
                HAVING SUM(b.base+nvl(b.descuento,0)) <> 0
                UNION ALL
                --OTROS_CARGOS
    ...
    //Ventas Transf Colegio
    public static function listVoucherCWAasinetSalesTCU($id_user,$id_entidad,$id_depto,$id_anho,$id_mes){
        $id_compra = "001-".$id_anho;
        $query = "SELECT 
                        VOUCHER,
                        to_char(fecha,'DD/MM/YYYY') FECHA,
                ...
                AND VOUCHER NOT IN (
                    ...
                    AND ID_AREA = '12'
                    AND ID_NIVEL = '4'
                    AND TIPO = 'RT' -- VENTAS TRANSFERENCIAS RT: registro transfe
                )
                ORDER BY fecha_ord DESC "; 
        $oQuery = DB::connection('oracleapp')->select($query);    
        return $oQuery;
    }
    public static function listCWSeatAaasinetTCU($id_nivel,$id_anho,$voucher){
        $id_nivel = "4";
        $mai_id = "001-".$id_anho;
        $query = "SELECT 
                        nvl(ARON.fc_cta_aasinet(id_cuenta_gasto,id_venta),' ') cuenta, 
                        nvl(ARON.fc_cta_cte_aasinet(id_cuenta_gasto,id_venta),' ') cta_cte, 
                        '10' fondo,
                        nvl(ARON.fc_restriccion_aasinet(id_cuenta_gasto,id_venta),' ')  restriccion,
                        nvl(ARON.equiv_nivel(decode(id_nivel_gasto,'4.02.01','4','4.02.02','4','4.02.03','4',id_nivel_gasto),id_venta),' ') departamento, 
                        trim(to_char(importe*decode(dc,'D',-1,1),'999999999.99')) valor, 
                        substr(glosa,1,30)||' (Doc:'||nvl(numtran,' ')||')' descripcion,
                        id_mov_tran,
                        '1' ORDEN
                FROM ARON.cu_transf 
                WHERE id_venta = '".$mai_id."'
                ...
                UNION ALL 
                SELECT 
                        nvl(ARON.fc_cta_aasinet(id_cuenta_cli,id_venta),' ') cuenta, 
                        nvl(ARON.fc_cta_cte_aasinet(id_cuenta_cli,id_venta),' ') cta_cte, 
                        '10' fondo,
                        nvl(ARON.fc_restriccion_aasinet(id_cuenta_cli,id_venta),' ')  restriccion,
                        nvl(ARON.equiv_nivel(nvl(decode(id_nivel_cli,'4.02.01','4','4.02.02','4','4.02.03','4',id_nivel_cli),'1'),id_venta),' ') departamento, 
                        to_char(sum(importe*decode(dc,'D',1,-1)),'99999999.99') valor,  
                        'Registro de Transferencias'||' 07 '||decode('11','01','Enero','02','Febrero','03','Marzo','04','Abril','05','Mayo','06','Junio','07','Julio','08','Agosto','09','Septiembre','10','Octubre','11','Noviembre','Diciembre') descripcion,
                        '' id_mov_tran,
                        '2' ORDEN
                FROM ARON.cu_transf 
                WHERE id_venta = '".$mai_id."'
                AND VOUCHER = '".$voucher."'
                AND estado = 'V' 
                GROUP BY id_venta,id_cuenta_cli,id_nivel_cli
                ORDER BY ORDEN,CUENTA  ";
        $oQuery = DB::connection('oracleapp')->select($query);    
        return $oQuery;
    }
    public static function uploadSeatAaasinetTCU($id_entidad,$id_nivel,$id_anho,$voucher,$numero,$fecha,$codigo,$fecha_aasi,$periodo,$url_aasinet,$descripcion, $certificado=""){
        $mai_id = "001-".$id_anho;
        $query = "SELECT                                
                        '".$url_aasinet."' as URL,
                        xmlelement(name \"Context\", xmlelement(name \"AccountingEntity\",".$id_entidad."),xmlelement(name \"Certificate\",'".$certificado."')) context,  
                    ...
                    FROM (
                                SELECT 
                                        ...
                                        nvl(ARON.equiv_nivel(decode(id_nivel_gasto,'4.02.01','4','4.02.02','4','4.02.03','4',id_nivel_gasto),id_venta),' ') departamento, 
                                        trim(to_char(importe*decode(dc,'D',-1,1),'999999999.99')) valor, 
                                        substr(glosa,1,30)||' (Doc:'||nvl(numtran,' ')||')' historico,
                                        id_mov_tran memo,
                                        '1' ORDEN
                                FROM ARON.cu_transf 
                                WHERE id_venta = '".$mai_id."'
                                ...
                                SELECT 
                                        ...
                                        nvl(ARON.equiv_nivel(nvl(decode(id_nivel_cli,'4.02.01','4','4.02.02','4','4.02.03','4',id_nivel_cli),'1'),id_venta),' ') departamento, 
                                        to_char(sum(importe*decode(dc,'D',1,-1)),'99999999.99') valor,  
                                        'Registro de Transferencias'||' 07 '||decode('11','01','Enero','02','Febrero','03','Marzo','04','Abril','05','Mayo','06','Junio','07','Julio','08','Agosto','09','Septiembre','10','Octubre','11','Noviembre','Diciembre') historico,
                                        '' memo,
                                        '2' ORDEN 
                                FROM ARON.cu_transf 
                                WHERE id_venta = '".$mai_id."'

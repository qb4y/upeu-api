...
<head>
    ...
    <link rel="stylesheet" href="{{asset('css/printlamb.css')}}">
...
<body>
    ...
    $logo =  \DB::table('ELISEO.CONTA_EMPRESA')->select('logo')
        ->where('ID_EMPRESA', $datos['id_empresa'])->pluck('logo')->first();
    ?>
    <header>
        <table>
            <tr>
                <td>
                  ...
                  @else
                    <img src="{{public_path($logo)}}" height="60" />
                  @endif
                  <?php
                ...
                <td style="width:100%; text-align: center"><h3>{{$datos['empresa']}}<br><span style="font-size: 10px;">{{$datos['depto']}}</span></h3></td>
                <td><img src="{{public_path('img/lamb.png')}}" class="img-rounded" /></td>
            </tr>
        </table>    
    </header>
    <footer>

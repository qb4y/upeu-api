...
<body>
    <div id="content">
        ...
        <br/><br/>
        Le informamos que el pedido <b>#{{ $nropedido }}</b> departamento <b>"{{ $departamento }}"</b> fecha <b>'{{ $fecha }}'</b> fue <b>Aceptado</b>. Puede seguir con el proceso hacia la <b>"Pre-Provision"</b>.
        
        <br/><br/>
        De presentarse algún inconveniente, por favor comunicarse a los siguientes correos: 
        
        <!-- Vilma Idrogo (Nombre del director de remuneraciones de sede)<br/>remuneraciones@upeu.edu.pe

        <br/><br/>        
        Área de Sistemas<br/>Sistemas.gth@upeu.edu.pe -->

        @foreach ($coreos_help as $coreo)
        <br/><br/>        
        <span>{{ $coreo["area"] }}</span>
        <br/>
        <span>{{ $coreo["cuenta"] }}</span>
        @endforeach
